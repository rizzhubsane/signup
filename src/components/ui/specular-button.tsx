...
export interface SpecularButtonProps {
  ...
  proximity?: number;
  autoAnimate?: boolean;
  /** Freeze WebGL when true (e.g. modal open). */
  paused?: boolean;
  disabled?: boolean;
...
export default function SpecularButton({
  ...
  useEffect(() => {
    // Touch / reduced-motion devices keep the static glass button — zero GPU.
    if (!canRunSpecularFx()) return;
    ...
    const update = (now: number) => {
      ...
      if (!p.autoAnimate && bright < 0.01 && proximityT < 0.01) {
        running = false;
        return;
      }
    ...
    const ensureLoop = () => {
      if (running || propsRef.current.paused || document.hidden) return;
      running = true;
